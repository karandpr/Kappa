...
out:
	...
	//if (unlikely(ioc->ioprio_changed))
        //test_and_clear_bit() implies a memory barrier, paired with the wmb() in fs/ioprio.c, so the value seen for ioprio is the new one.
        
        if (unlikely(test_and_clear_bit(IOC_CFQ_IOPRIO_CHANGED, ioc->ioprio_changed)))
